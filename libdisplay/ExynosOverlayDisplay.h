...
class ExynosOverlayDisplay : public ExynosDisplay {
    public:
        ...
        struct decon_win_config mLastConfig[NUM_HW_WINDOWS];
    ...
    protected:
        ...
        void configureHandle(private_handle_t *handle, uint32_t unknown, hwc_layer_1 &layer, int fence_fd, decon_win_config &cfg, enum decon_idma_type idma_type);
        int clearDisplay();
        ...
        int postGscOtf(hwc_layer_1_t &layer, struct decon_win_config *config, int win_map, int index);
        void handleStaticLayers(hwc_display_contents_1_t *contents, struct decon_win_config_data &win_data, int tot_ovly_wins);
        void cleanupGscs();

        virtual int postGscM2M(hwc_layer_1_t &layer, struct decon_win_config *config, int win_map, int index);
        virtual void forceYuvLayersToFb(hwc_display_contents_1_t *contents);
        virtual void configureOverlay(hwc_layer_1_t *layer, decon_win_config &cfg, enum decon_idma_type idma_type);
