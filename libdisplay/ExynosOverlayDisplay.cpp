...
int ExynosOverlayDisplay::prepare(hwc_display_contents_1_t* contents)
{
    ...
    if (mBypassCount > 0) {
        ALOGD_IF(debug_level > 0, "%s: mBypassCount(%d)", __FUNCTION__, mBypassCount);
        mForceFb = true;
    }
...
// TODO: add skip static layers logic (odroid sources look good)
    ...
    ALOGD_IF(debug_level > 0, "%s: mNeededRgbOvly(%d) mNeededIntOvly(%d)", __FUNCTION__, mNeededRgbOvly, mNeededIntOvly);
...
{
    ALOGD_IF(debug_level > 0, "%s", __FUNCTION__);
...
int ExynosOverlayDisplay::postFrame(hwc_display_contents_1_t* contents)
{
    ...
    for (size_t i = 0; i < NUM_HW_WINDOWS; i++) {
        ...
        if (layer_idx != -1) {
            ...
            if (pdata->gsc_map[i].mode == exynos5_gsc_map_t::GSC_M2M) {
                ...
                ALOGD_IF(debug_level > 0, "%s: layer %u win_map(%d) idma_type[%d]=%d", __FUNCTION__, layer_idx, win_map, layer_idx, (int)pdata->decon_dma_type[layer_idx]);
            ...
            if (win_map == 0 && config[win_map].blending != DECON_BLENDING_NONE) {
                ALOGD_IF(debug_level > 0, "blending not supported on window 0; forcing BLENDING_NONE");
    ...
    for (size_t i = 0; i < NUM_HW_WINDOWS; i++) {
        ALOGD_IF(debug_level > 0, "window %u configuration:", i);
        dumpConfig(config[i]);
    }
    ...
    if (this->mPostData.fb_window != NO_FB_NEEDED) {
        for (size_t i = 0; i < contents->numHwLayers; i++) {
            if (contents->hwLayers[i].compositionType == HWC_FRAMEBUFFER_TARGET) {
                ...
                this->mPostData.decon_dma_type[i] = getDeconDMAType(DECON_RGB, this->mPostData.numRgbOvly);
                ALOGD_IF(debug_level > 0, "%s: i(%d) fb_window(%d) framebuffer decon_dma_type[%d]=%d", __FUNCTION__, i, this->mPostData.fb_window, i, this->mPostData.decon_dma_type[i]);
...
int ExynosOverlayDisplay::getCompModeSwitch()
{
    ...
    if (!mHwc->hwc_ctrl.dynamic_recomp_mode) {
        ...
        ALOGD_IF(debug_level > 0, "%s: dynamic_recomp_mode is 0", __FUNCTION__);
        return 0;
    }
    ...
    for (size_t i = 0; i < NUM_HW_WINDOWS; i++) {
        if (this->mLastGscMap[i].mode != exynos5_gsc_map_t::GSC_NONE) {
            if (mHwc->CompModeSwitch != HWC_2_GLES) {
                ALOGD_IF(debug_level > 0, "%s: != GSC_NONE return 0", __FUNCTION__);
                return 0;
            } else {
                ...
                ALOGD_IF(debug_level > 0, "%s: != GSC_NONE GLES_2_HWC by video layer", __FUNCTION__);
                return GLES_2_HWC;
            }
        }
    }
    ...
    if ((uint32_t)mHwc->totPixels <= lcd_size * HWC_FIMD_BW_TH) {
        if (mHwc->CompModeSwitch != HWC_2_GLES) {
            ALOGD_IF(debug_level > 0, "%s: totPixels(%d) <= return 0", __FUNCTION__);
            return 0;
        } else {
            mHwc->CompModeSwitch = GLES_2_HWC;
            mHwc->updateCallCnt = 0;
            ...
            ALOGD_IF(debug_level > 0, "%s: totPixels <= GLES_2_HWC by BW check", __FUNCTION__);
            return GLES_2_HWC;
        }
    }
    ...
    if (updateFps < HWC_FPS_TH) {
        if (mHwc->CompModeSwitch != HWC_2_GLES) {
            ...
            ALOGD_IF(debug_level > 0, "%s: HWC_2_GLES by low FPS(%d)", __FUNCTION__, updateFps);
            return HWC_2_GLES;
        } else {
            ALOGD_IF(debug_level > 0, "%s: updateFps < HWC_FPS_TH return 0", __FUNCTION__);
            return 0;
        }
    } else {
         if (mHwc->CompModeSwitch == HWC_2_GLES) {
            ...
            ALOGD_IF(debug_level > 0, "%s: GLES_2_HWC by high FPS(%d)", __FUNCTION__, updateFps);
            return GLES_2_HWC;
        } else {
            ALOGD_IF(debug_level > 0, "%s: CompModeSwitch == HWC_2_GLES return 0", __FUNCTION__);
            return 0;
        }
    }

    ALOGD_IF(debug_level > 0, "%s: just return 0", __FUNCTION__);
    return 0;
}
...
void ExynosOverlayDisplay::determineSupportedOverlays(hwc_display_contents_1_t *contents)
{
    ...
    for (size_t i = 0; i < contents->numHwLayers; i++) {
        ...
        if (layer.handle) {
            ...
            if (isOverlaySupported(contents->hwLayers[i], i) && !mForceFb) {
                ALOGD_IF(debug_level > 0, "\tlayer %u: overlay supported", i);
                layer.compositionType = HWC_OVERLAY;
                dumpLayer(&contents->hwLayers[i]);
                continue;
            }
    ...
    if (mFbNeeded) {
        for (size_t i = mFirstFb; i < mLastFb; i++) {
            ALOGD_IF(debug_level > 0, "%s layer %d sandwiched between FB, set to FB too", __FUNCTION__, i);
...
void ExynosOverlayDisplay::determineBandwidthSupport(hwc_display_contents_1_t *contents)
{
    ...
    do {
        ...
        for (size_t i = 0; i < contents->numHwLayers; i++) {
            ...
            ALOGD_IF(debug_level > 0, "%s: can_compose_rgb(%d) gsc_required(%d)", __FUNCTION__, can_compose_rgb, gsc_required);
...
void ExynosOverlayDisplay::assignWindows(hwc_display_contents_1_t *contents)
{
        ...
        if (layer.compositionType != HWC_FRAMEBUFFER &&
            ...
            if (layer.compositionType == HWC_OVERLAY) {
                ...
                if (mMPPs[0]->isProcessingRequired(layer, handle->format)) {
                    ALOGD_IF(debug_level > 0, "%s processing required, format(0x%x)", __FUNCTION__, handle->format);
                    ...
                    /*if (isTransformed(layer)) {
                        ...
                        if (isFormatRgb(handle->format)) { //TODO: use maybe isSupportedByDecon()
                            if (this->mPostData.numRgbOvly < NUM_RGB_HW_WINDOWS) {
                                this->mPostData.decon_dma_type[i] = getDeconDMAType(DECON_RGB, this->mPostData.numRgbOvly);
                                ALOGD_IF(debug_level > 0, "%s: rgb format decon_dma_type[%d]=%d", __FUNCTION__, i, this->mPostData.decon_dma_type[i]);
                                this->mPostData.numRgbOvly++;
                            } else {
                                //use internal MPP for compositing rgb
                                this->mPostData.decon_dma_type[i] = getDeconDMAType(DECON_MPP_INT, this->mPostData.numIntOvly);
                                ALOGD_IF(debug_level > 0, "%s: rgb using MPP decon_dma_type[%d]=%d", __FUNCTION__, i, this->mPostData.decon_dma_type[i]);
                                this->mPostData.numIntOvly++;
                            }
                        } else {
                            ALOGD_IF(debug_level > 0, "%s window %u processing not required, format is not RGB (0x%x)", __FUNCTION__, nextWindow, handle->format);
                        }
    ...
    ALOGD_IF(debug_level > 0, "%s: Assigned %u windows", __FUNCTION__, nextWindow);
}
