...
void ExynosOverlayDisplay::configureHandle(private_handle_t *handle,
        uint32_t unknown, hwc_layer_1 &layer, int fence_fd, decon_win_config &cfg)
{
    uint32_t x, y;
    ...
    //J.Ferrer - uncomment it when using decon win config
    /*cfg.plane_alpha = 255;
    if (planeAlpha && (planeAlpha < 255)) {
        cfg.plane_alpha = planeAlpha;
    }*/
...
void ExynosOverlayDisplay::configureOverlay(hwc_layer_1_t *layer, decon_win_config &cfg)
{
    ...
    private_handle_t *handle = private_handle_t::dynamicCast(layer->handle);
    configureHandle(handle, 0, *layer, layer->acquireFenceFd, cfg);
}
...
int ExynosOverlayDisplay::postGscM2M(hwc_layer_1_t &layer, struct decon_win_config *config, int win_map, int index)
{
    ...
    configureHandle(dst_handle, 0, layer, fence, config[win_map]);
    return 0;
}
