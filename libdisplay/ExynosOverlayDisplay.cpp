...
/*    uint8_t bpp = formatToBpp(format);
    ...
    cfg.state = cfg.DECON_WIN_STATE_OTF;
    cfg.fd = -1;
    ...
    for (size_t i = 0; i < NUM_HW_WINDOWS; i++) {
        ...
        if (layer_idx != -1) {
            ...
            /* } else if (this->mOtfMode == OTF_RUNNING &&
                ...
                configureHandle(&dstHandle, 0, layer, fence, *config);
                num_g2d_overlayed++;
...
int ExynosOverlayDisplay::set(hwc_display_contents_1_t* contents)
{
    ...
    if (this->mPostData.fb_window != NO_FB_NEEDED) {
        ...
        if (CC_UNLIKELY(!fb_layer)) {
            ...
            ALOGD("%s: framebuffer target buffer:", __FUNCTION__);
            dumpLayer(fb_layer);
        }
    }
        ...
        if (layer.compositionType != HWC_FRAMEBUFFER &&
            ...
            if (layer.compositionType == HWC_OVERLAY) {
                ...
                if (mMPPs[0]->isProcessingRequired(layer, handle->format)) {
                    ...
                    ALOGD("%s processing not required, format(0x%x)", __FUNCTION__, handle->format);
                    if (isFormatRgb(handle->format)) {
                        this->mPostData.decon_dma_type[nextWindow] = getRGBDeconDMAType(this->mPostData.numRgbOvly);
                        ALOGD("%s: rgb format decon_dma_type[%d]=%d", __FUNCTION__, nextWindow, this->mPostData.decon_dma_type[nextWindow]);
                        this->mPostData.numRgbOvly++;
                    }
                }
            }
