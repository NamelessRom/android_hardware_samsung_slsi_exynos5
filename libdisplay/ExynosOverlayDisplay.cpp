...
int ExynosOverlayDisplay::postFrame(hwc_display_contents_1_t* contents)
{
    ...
    for (size_t i = 0; i < NUM_HW_WINDOWS; i++) {
        ...
        if (layer_idx != -1) {
            ...
            LOG_SET("%s: layer %u getDeconWinMap(%d)", __FUNCTION__, layer_idx, getDeconWinMap(i, tot_ovly_wins));
            ...
            if (pdata->gsc_map[i].mode == exynos5_gsc_map_t::GSC_M2M) {
                ...
                LOG_SET("%s: layer %u win_map(%d) idma_type[%d]=%d", __FUNCTION__, layer_idx, win_map, layer_idx, (int)pdata->decon_dma_type[layer_idx]);
    ...
    if (this->mPostData.fb_window != NO_FB_NEEDED) {
        for (size_t i = 0; i < contents->numHwLayers; i++) {
            if (contents->hwLayers[i].compositionType == HWC_FRAMEBUFFER_TARGET) {
                ...
                LOG_SET("%s: i(%d) fb_window(%d) framebuffer decon_dma_type[%d]=%d", __FUNCTION__, i, this->mPostData.fb_window, i, this->mPostData.decon_dma_type[i]);
            ...
            if (layer.compositionType == HWC_OVERLAY) {
                ...
                if (mMPPs[0]->isProcessingRequired(layer, handle->format)) {
                    ...
                    if (isFormatSupported(handle->format)) { //TODO: use maybe isSupportedByDecon()
                        ...
                        LOG_ASSIGN("%s: ROTATED rgb format decon_dma_type[%d]=%d", __FUNCTION__, i, this->mPostData.decon_dma_type[i]);
                    ...
                    /*if (isTransformed(layer)) {
                        ...
                        if (isFormatRgb(handle->format)) { //TODO: use maybe isSupportedByDecon()
                            if (this->mPostData.numRgbOvly < availRGBwin) {
                                this->mPostData.decon_dma_type[i] = getDeconDMAType(DECON_RGB, this->mPostData.numRgbOvly);
                                LOG_ASSIGN("%s: rgb format decon_dma_type[%d]=%d", __FUNCTION__, i, this->mPostData.decon_dma_type[i]);
                                this->mPostData.numRgbOvly++;
                            } else {
                                //we've used all RGB overlays possible, use internal MPP for compositing rgb
                                this->mPostData.decon_dma_type[i] = getDeconDMAType(DECON_MPP_INT, this->mPostData.numIntOvly);
                                LOG_ASSIGN("%s: rgb using MPP decon_dma_type[%d]=%d", __FUNCTION__, i, this->mPostData.decon_dma_type[i]);
                                this->mPostData.numIntOvly++;
