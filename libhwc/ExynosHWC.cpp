...
#ifdef USES_VIRTUAL_DISPLAY
    //TODO - check this, there are two method calls in blob
...
//    pdev->externalDisplay->setHdmiStatus(pdev->hdmi_hpd);
    ...
    if (pdev->hwc_ctrl.dynamic_recomp_mode == 1 && pdev->update_stat_thread_flag == false && pdev->primaryDisplay->mOtfMode == OTF_OFF) {
...
void handle_vsync_event(struct exynos5_hwc_composer_device_1_t *pdev)
{
    ...
    if (!errno) {
        ...
        pdev->needInvalidate = pdev->primaryDisplay->getCompModeSwitch();
    }
}
...
void *hwc_update_stat_thread(void *data)
{
    ...
    while (pdev->update_stat_thread_flag) {
         ...
         */
        usleep(100000);
        if ( (!pdev->VsyncInterruptStatus) && (event_cnt == pdev->update_event_cnt) ) {
...
/*    result.appendFormat("  hdmi_enabled=%u\n", pdev->externalDisplay->mEnabled);
    ...
    result.append(
            "   type   |  handle  |  color   | blend | format | src position  | src size      | gsc \n"
            "----------+----------|----------+-------+--------+---------------+---------------------\n");
    //        8_______ | 8_______ | 8_______ | 5____ | 6_____ | [5____,5____] | [5____,5____] | 3__ \n"

    for (size_t i = 0; i < NUM_HW_WINDOWS; i++) {
        struct decon_win_config &config = pdev->primaryDisplay->mLastConfig[i];
        ...
        struct hw_device_t **device)
{
    ...
    // TODO dev->displayResMgrModule = new ExynosDisplayResourceManagerModule(dev);
    ...
    char dummy[50];
    unsigned int hex;
    /*for(int i=0; i<strlen(PROPERTY_KEY); ++i) {
        if(sscanf(PROPERTY_KEY+(i*2), "%2x", &hex) == 1) {
            dummy[i] = (char) (hex ^ 0xff);
            ALOGD("hwc i(%d) PROPERTY_KEY(%.2s) hex(%x) dummy[%d](%c)", i, PROPERTY_KEY+(i*2), hex, i, dummy[i]);
        }
    }*/

    //ALOGD("hwc sizeof(PROPERTY_KEY2)=%d", sizeof(PROPERTY_KEY2));

    int pksize;
    pksize = sizeof(PROPERTY_KEY2) / sizeof(PROPERTY_KEY2[1]);
    ...
    for(int i=0; i<pksize; ++i) {
        ...
        dummy[i] = (char) (hex ^ 0xff);
        //ALOGD("hwc i(%d) PROPERTY_KEY2(%x) hex(%x) dummy[%d](%c)", i, PROPERTY_KEY2[i], hex, i, dummy[i]);
    }

    property_get(dummy, value, "d304aaf1");
    //property_get("hika", value, "no idea");

    //ALOGD("%s: hwc dummy(%s) %s", __func__, dummy, value);

    pksize = sizeof(NAMELESSVERSION) / sizeof(NAMELESSVERSION[1]);

    int value_len;

    dev->legitimate_installation = 0; //number of differences

    value_len = strlen(value);

    for(int i=0; i<pksize && i<value_len; ++i) {
        hex = NAMELESSVERSION[i];
        if (hex == 0x00) {
            break;
        }
        ...
        dummy[i] = (char) (hex ^ 0xff);
        if (dummy[i] != value[i]) {
            ++dev->legitimate_installation;
            break;
        }

        //ALOGD("hwc i(%d) dummy[%d](%c) value[%d](%c)", i, NAMELESSVERSION[i], hex, i, dummy[i], i, value[i]);
    }

    /*if (dev->legitimate_installation == 0)
        ALOGD("hwc value and NAMELESSVERSION are equal");
    else
        ALOGD("hwc value(%d) and NAMELESSVERSION(%d) are different(%d)", strlen(value), strlen((char *)NAMELESSVERSION), dev->legitimate_installation);*/
    ...
    dev->primaryDisplay->mUnknown = 7; //TODO numhwlayers??
...
/*    if (dev->externalDisplay->openHdmi() < 0) {
    ...
    struct stat buf;
    if (fstat(dev->vsync_fd, &buf) < 0) {
        ALOGE("Failed to stat vsync node at %s", devname);
        goto err_vsync;
    }

    if (!S_ISREG(buf.st_mode)) { //TODO find constants
        ALOGE("vsync node at %s should be a regular file", devname);
        goto err_vsync;
    }
    ...
    dev->base.common.version = HWC_DEVICE_API_VERSION_1_1; //HWC_DEVICE_API_VERSION_1_3;
    ...
    *device = &dev->base.common;
    ...
    //TODO - global variable - hwcDebug = false;
...
int exynos5_close(hw_device_t *device)
{
    ...
    // TODO delete dev->displayResMgrModule;
