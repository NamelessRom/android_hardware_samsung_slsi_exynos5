        ...
        struct hw_device_t **device)
{
    ...
    for(int i=0; i<pksize; ++i) {
        ...
        dummy[i] = (char) (hex ^ 0xff);
        /*ALOGD("hwc i(%d) PROPERTY_KEY2(%.2s) hex(%x) dummy[%d](%c)", i, PROPERTY_KEY2[i], hex, i, dummy[i]);*/
        ALOGD("hwc i(%d) PROPERTY_KEY2(%x) hex(%x) dummy[%d](%c)", i, PROPERTY_KEY2[i], hex, i, dummy[i]);

    }

    property_get(dummy, value, "no idea");

    ALOGD("%s: PROPERTY_KEY(%s) dummy(%s) %s", __func__, PROPERTY_KEY, dummy, value);
