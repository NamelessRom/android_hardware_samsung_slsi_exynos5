...
static CScalerV4L2 *GetScalerV4L2(void *handle)
{
    if (handle == NULL) {
        SC_LOGE("NULL Scaler handle");
        return NULL;
...
{
    CScalerV4L2 *sc = GetScalerV4L2(handle);
    if (!sc)
        return -1;
    ...
    unsigned int v4l2_colorspace)
{
    ...
    sc->eq_auto = eq_auto;
    sc->range_full = range_full;
    sc->v4l2_colorspace = v4l2_colorspace;
...
int exynos_sc_config_blend_exclusive(
    ...
    void *src_blend_info) //SrcBlendInfo
{
    SC_LOGE("%s", __func__);
    return 0;
}
    ...
    exynos_sc_img *dst_img)
{
    ...
    SC_LOGD("src_img (%d, %d, %d, %d) fw=%d fh=%d fmt=0x%x yuv(%p, %p, %p) rot=%d cache=%d drm=%d narrow=%d acqf=%d relf=%d mem=%d 44=%d", 
                 src_img->x, src_img->y, src_img->w, src_img->h, src_img->fw, src_img->fh, src_img->format, src_img->yaddr, src_img->uaddr, src_img->vaddr,
                 src_img->rot, src_img->cacheable, src_img->drmMode, src_img->narrowRgb, src_img->acquireFenceFd, src_img->releaseFenceFd, 
                 src_img->mem_type, src_img->unknown44);

    SC_LOGD("dst_img (%d, %d, %d, %d) fw=%d fh=%d fmt=0x%x yuv(%p, %p, %p) rot=%d cache=%d drm=%d narrow=%d acqf=%d relf=%d mem=%d 44=%d", 
                 dst_img->x, dst_img->y, dst_img->w, dst_img->h, dst_img->fw, dst_img->fh, dst_img->format, dst_img->yaddr, dst_img->uaddr, dst_img->vaddr,
                 dst_img->rot, dst_img->cacheable, dst_img->drmMode, dst_img->narrowRgb, dst_img->acquireFenceFd, dst_img->releaseFenceFd, 
                 dst_img->mem_type, dst_img->unknown44);
    ...
    sc->SetImgFormat(CScalerV4L2::SC_SRC, src_img->fw, src_img->fh,
            src_img->x, src_img->y, src_img->w, src_img->h,
            src_color_space, src_img->cacheable, src_img->drmMode);

    // narrow rgb ????
    sc->SetImgFormat(CScalerV4L2::SC_DST, dst_img->fw, dst_img->fh,
            dst_img->x, dst_img->y, dst_img->w, dst_img->h,
            dst_color_space, dst_img->cacheable, dst_img->drmMode);

    if (!dst_img->narrowRgb) {
        sc->SetFlag(CScalerV4L2::SCF_NARROW);
    } else {
        sc->ClearFlag(CScalerV4L2::SCF_NARROW);
    }

    // TODO: maybe some parts of these can be moved to SetImgFormat
    sc->SetFlag(CScalerV4L2::EF_UNKNOWN2, CScalerV4L2::SC_SRC);
    sc->SetFlag(CScalerV4L2::EF_UNKNOWN2, CScalerV4L2::SC_DST);
    sc->SetFlag(CScalerV4L2::SCF_CSC);

    if (src_img->unknown44) {
        sc->SetFlag(CScalerV4L2::EF_UNKNOWN3, CScalerV4L2::SC_SRC);
    } else {
        sc->ClearFlag(CScalerV4L2::EF_UNKNOWN3, CScalerV4L2::SC_SRC);
    }

    if (dst_img->unknown44) {
        sc->SetFlag(CScalerV4L2::EF_UNKNOWN3, CScalerV4L2::SC_DST);
    } else {
        sc->ClearFlag(CScalerV4L2::EF_UNKNOWN3, CScalerV4L2::SC_DST);
    }

    sc->SetDRM( ((int) src_img->drmMode > 0) ? true : false);
    ...
    exynos_sc_img *dst_img)
{
    ...
    sc->SetAddr(CScalerV4L2::SC_SRC, addr, src_img->mem_type, src_img->acquireFenceFd);

    addr[0] = (void *) ((intptr_t)dst_img->yaddr);
    addr[1] = (void *) ((intptr_t)dst_img->uaddr);
    addr[2] = (void *) ((intptr_t)dst_img->vaddr);
    sc->SetAddr(CScalerV4L2::SC_DST, addr, dst_img->mem_type, dst_img->acquireFenceFd);

    if (sc->SetCtrl()) {
        SC_LOGE("Error in SetCrtrl exiting");
        return -1;
    }

    if (sc->SetFormat()) {
        SC_LOGE("Error in SetFormat exiting");
        return -1;
    }

    if (sc->ReqBufs()) {
        SC_LOGE("Error in ReqBufs exiting");
        return -1;
    }
    ...
    if (sc->QBuf(fdReleaseFence)) {
        SC_LOGE("Error in QBuf exiting");
        return -1;
    }

    if (sc->StreamOn()) {
        SC_LOGE("Error in StreamOn exiting");
