    ...
    for (int edge = 0; edge < SC_NUM_EDGES; edge++) {
        ...
        for (unsigned long i = 0; i < buffer.length; i++) {
            ...
            if (buffer.memory == V4L2_MEMORY_DMABUF)
                planes[i].m.fd = (int) ((intptr_t)m_Frame.edge[edge].addr[i]);
            else
...
public:
    ...
    enum SC_FLAG {
        SCF_ROTATION_FRESH = 1, // confirm
        SCF_CSC = 2,
        SCF_DRM_FRESH = 3, //DRM mode should be send to v4l2
        SCF_VFLIP = 4,
        SCF_HFLIP = 5,
        SCF_DRM_ACTIVE = 6,
        SCF_ALLOW_DRM = 7,
...
private:
    ...
    bool IsSet(SC_FLAG flag) {
        return m_Frame.flags & (1 << flag);
    }

    //for edge flags
    bool IsSet(E_FLAG flag, int edge) {
        return m_Frame.edge[edge].flags & (1 << flag);
    }

    void SetRotDegree(int rot) {
        m_rot = rot - ((rot / 360) * 360);
        if (m_rot < 0)
            m_rot = 360 + rot;

        SC_LOGD("rot(%d) m_rot(%d)", rot, m_rot);
...
public:
    ...
    CScalerV4L2(int instance, int allow_drm);
    ~CScalerV4L2();
    ...
    inline void SetAddr(SC_EDGE edge, void *addr[SC_NUM_OF_PLANES], int mem_type, int fence = -1)
    {
        SC_LOGD("edge(%d) addr[0]=0x%x addr[1]=0x%x addr[2]=0x%x mem_type=%d fence=%d",
                 edge, addr[0], addr[1], addr[2], mem_type, fence);
    ...
    // needed for exynos_sc_config_exclusive()
    void SetFlag(SC_FLAG flag) {
        m_Frame.flags |= 1 << flag;
    }

    void ClearFlag(SC_FLAG flag) {
        m_Frame.flags &= ~(1 << flag);
    }

    void SetFlag(E_FLAG flag, int edge) {
        m_Frame.edge[edge].flags |= 1 << flag;
    }

    void ClearFlag(E_FLAG flag, int edge) {
        m_Frame.edge[edge].flags &= ~(1 << flag);
    }
};
...
void CScalerV4L2::Initialize(int instance)
{
    ...
    if (m_fdScaler < 0) {
        SC_LOGERR("%s: Failed to open '%s'", __FUNCTION__, m_cszNode);
        return;
    }
    ...
    unsigned int cap = V4L2_CAP_STREAMING | V4L2_CAP_VIDEO_OUTPUT_MPLANE |
                        V4L2_CAP_VIDEO_CAPTURE_MPLANE;
    if (!exynos_v4l2_querycap(m_fdScaler, cap)) {
        SC_LOGERR("%s: Failed to query capture on '%s'", __FUNCTION__, m_cszNode);
        close(m_fdScaler);
        m_fdScaler = -1;
    } else {
        m_fdValidate = -m_fdScaler;
...
CScalerV4L2::CScalerV4L2(int instance, int allow_drm)
{
    ...
    m_Frame.edge[SC_SRC].name = "output";
    m_Frame.edge[SC_SRC].type = V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE;
    m_Frame.edge[SC_SRC].fdAcquireFence = -1;

    m_Frame.edge[SC_DST].name = "capture";
    m_Frame.edge[SC_DST].type = V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE;
    m_Frame.edge[SC_DST].fdAcquireFence = -1;
...
int CScalerV4L2::Stop()
{
    SC_LOGD();
    for (int edge = 0; edge < SC_NUM_EDGES; edge++) {
        if (ResetDevice(static_cast<SC_EDGE>(edge)) != 0) {
            SC_LOGE("Failed to stop Scaler %s", m_Frame.edge[edge].name);
            return -1;
        }
    }
...
int CScalerV4L2::SetCtrl()
{
    SC_LOGD();
    if (IsSet(SCF_DRM_FRESH)) {
        if (Stop() < 0) {
            SC_LOGE("Couldn't stop to set V4L2_CID_CONTENT_PROTECTION, exiting");
            return 0;
        }

        if (exynos_v4l2_s_ctrl(m_fdScaler, V4L2_CID_CONTENT_PROTECTION, IsSet(SCF_DRM_ACTIVE)) < 0) {
            SC_LOGERR("Failed V4L2_CID_CONTENT_PROTECTION");
            return -1;
        }

        ClearFlag(SCF_DRM_FRESH);
    }

    if (IsSet(SCF_ROTATION_FRESH)) {
        if (Stop() < 0) {
            SC_LOGE("Couldn't stop to set rotation and flip, exiting");
            return 0;
        }

        if (exynos_v4l2_s_ctrl(m_fdScaler, V4L2_CID_ROTATE, m_rot) < 0) {
            SC_LOGERR("Failed V4L2_CID_ROTATE with degree %d", m_rot);
            return -1;
        }

        if (exynos_v4l2_s_ctrl(m_fdScaler, V4L2_CID_VFLIP, IsSet(SCF_VFLIP)) < 0) {
            SC_LOGERR("Failed V4L2_CID_VFLIP - %d", IsSet(SCF_VFLIP));
            return -1;
        }

        if (exynos_v4l2_s_ctrl(m_fdScaler, V4L2_CID_HFLIP, IsSet(SCF_HFLIP)) < 0) {
            ALOGE("Failed V4L2_CID_HFLIP - %d", IsSet(SCF_HFLIP));
            return -1;
        }

        SC_LOGD("Successfully set CID_ROTATE(%d), CID_VFLIP(%d) and CID_HFLIP(%d)",
                m_rot, IsSet(SCF_VFLIP), IsSet(SCF_HFLIP));
    ...
    if (m_dnoise_ft) {
       ...
       if (exynos_v4l2_s_ctrl(m_fdScaler, SC_CID_DNOISE_FT, m_dnoise_ft) < 0) {
            SC_LOGERR("Failed SC_CID_DNOISE_FT with %d", m_dnoise_ft);
            return -1;
        }
    }
...
int CScalerV4L2::ResetDevice(SC_EDGE edge)
{
    SC_LOGD("edge name=\"%s\"", m_Frame.edge[edge].name);

    DQBuf(static_cast<SC_EDGE>(edge));

    if (IsSet(EF_STREAMING, edge)) {
        if (exynos_v4l2_streamoff(m_fdScaler, m_Frame.edge[edge].type) < 0 ) {
            SC_LOGERR("Failed STREAMOFF for the %s", m_Frame.edge[edge].name);
            return -1;
        }
        ClearFlag(EF_STREAMING, edge);
    }

    SC_LOGD("VIDIC_STREAMOFF is successful for the %s", m_Frame.edge[edge].name);

    if (IsSet(EF_REQBUFS, edge)) {
        v4l2_requestbuffers reqbufs;
        memset(&reqbufs, 0, sizeof(reqbufs));
        reqbufs.type = m_Frame.edge[edge].type;
        reqbufs.memory = m_Frame.edge[edge].memory;
        if (exynos_v4l2_reqbufs(m_fdScaler, &reqbufs) < 0 ) {
            SC_LOGERR("Failed to REQBUFS(0) for the %s", m_Frame.edge[edge].name);
            return -1;
        }

        ClearFlag(EF_REQBUFS, edge);
    }

    SC_LOGD("VIDIC_REQBUFS(0) is successful for the %s", m_Frame.edge[edge].name);

    return 0;
}

int CScalerV4L2::SetFormat()
{
    SC_LOGD();

    for (int edge = 0; edge < SC_NUM_EDGES; edge++) {
        if (!IsSet(EF_BUF_FRESH, edge)) {
            SC_LOGD("Skipping S_FMT for the %s since it is already done", m_Frame.edge[edge].name);
            continue; // No new format or control variable is set
        }

        if (ResetDevice(static_cast<SC_EDGE>(edge)) != 0) {
            SC_LOGE("Failed to VIDIOC_S_FMT for the %s", m_Frame.edge[edge].name);
            return -1;
        }
        ...
        fmt.fmt.pix_mp.width  = m_Frame.edge[edge].width;
        fmt.fmt.pix_mp.height = m_Frame.edge[edge].height;
        //TODO: there is something with IsSet(EF_UNKNOWN2, edge) in blob

        if (exynos_v4l2_s_fmt(m_fdScaler, &fmt) < 0) {
            SC_LOGERR("Failed S_FMT(fmt: %d, w:%d, h:%d) for the %s",
                    fmt.fmt.pix_mp.pixelformat, fmt.fmt.pix_mp.width, fmt.fmt.pix_mp.height,
                    m_Frame.edge[edge].name);
            return -1;
        }
        ...
        ClearFlag(EF_BUF_FRESH, edge);
        SC_LOGD("Successfully S_FMT and S_CROP for the %s", m_Frame.edge[edge].name);
    }

    return 0;
}

int CScalerV4L2::QBuf(int pfdReleaseFence[SC_NUM_EDGES])
{
    v4l2_buffer buffer;
    v4l2_plane planes[SC_MAX_PLANES];
    ...
    for (int edge = 0; edge < SC_NUM_EDGES; edge++) {
        ...
        if (DQBuf(static_cast<SC_EDGE>(edge)) < 0) {
            SC_LOGE("Error in DQBuf");
            return -1;
        }
        ...
        for (unsigned long i = 0; i < buffer.length; i++) {
            ...
            if (V4L2_TYPE_IS_OUTPUT(buffer.type))
                planes[i].bytesused = planes[i].length;
            if (buffer.memory == V4L2_MEMORY_DMABUF)
                planes[i].m.fd = (int) ( reinterpret_cast<intptr_t>(m_Frame.edge[edge].addr[i]) );
            else
                planes[i].m.userptr = reinterpret_cast<unsigned long>(m_Frame.edge[edge].addr[i]);
        }
        ...
        if (exynos_v4l2_qbuf(m_fdScaler, &buffer) < 0) {
            SC_LOGERR("Failed to QBUF for the %s", m_Frame.edge[edge].name);
            return -1;
        }

        SetFlag(EF_QBUF, edge);
        ...
        pfdReleaseFence[edge] = static_cast<int>(buffer.reserved);
        SC_LOGD("Successfully QBUF for the %s", m_Frame.edge[edge].name);
    }

    return 0;
}

int CScalerV4L2::ReqBufs()
{
    v4l2_requestbuffers reqbufs;
    ...
    for (int edge = 0; edge < SC_NUM_EDGES; edge++) {
        if (IsSet(EF_REQBUFS, edge)) {
            SC_LOGD("Skipping REQBUFS for the %s since it is already done", m_Frame.edge[edge].name);
            continue;
        }
        ...
        if (exynos_v4l2_reqbufs(m_fdScaler, &reqbufs) < 0) {
            SC_LOGERR("Failed to REQBUFS for the %s", m_Frame.edge[edge].name);
            return -1;
        }

        SetFlag(EF_REQBUFS, edge);

        SC_LOGD("Successfully REQBUFS for the %s", m_Frame.edge[edge].name);
    }
    return 0;
}

void CScalerV4L2::SetImgFormat(
        SC_EDGE edge,
        unsigned int width,
        unsigned int height,
        unsigned int crop_left,
        ...
        unsigned int premultiplied)
{
    SC_LOGD("edge(%d) (%d,%d) crop(%d,%d,%d,%d) format(0x%x) cache(%d) drm(%d) premulti(%d)",
        edge, width, height, crop_left, crop_top, crop_width, crop_height, v4l2_colorformat, cacheable, mode_drm, premultiplied);
...
{
    SC_LOGD("rot(%d) flip_h(%d) flip_v(%d)", rot, flip_h, flip_v);
...
int CScalerV4L2::StreamOn()
{
    ...
    for (int edge = 0; edge < SC_NUM_EDGES; edge++) {
        ...
        if (!IsSet(EF_STREAMING, edge)) {
            if (exynos_v4l2_streamon(m_fdScaler, m_Frame.edge[edge].type) < 0 ) {
                SC_LOGERR("Failed StreamOn for the %s", m_Frame.edge[edge].name);
                return errno;
            }

            SetFlag(EF_STREAMING, edge);

            SC_LOGD("Successfully VIDIOC_STREAMON for the %s", m_Frame.edge[edge].name);
        }
    }
...
int CScalerV4L2::DQBuf(SC_EDGE edge)
{
    SC_LOGD();

    if (!IsSet(EF_QBUF, edge))
        return 0;
    ...
    if (exynos_v4l2_dqbuf(m_fdScaler, &buffer) < 0 ) {
        SC_LOGERR("Failed to DQBuf the %s", m_Frame.edge[edge].name);
        return -1;
    }
    ...
    ClearFlag(EF_QBUF, edge);  //not in blob

    SC_LOGD("Successfully VIDIOC_DQBUF for the %s", m_Frame.edge[edge].name);

    return 0;
}

void CScalerV4L2::SetDRM(bool drm){
    SC_LOGD("drm=%s", drm ? "true" : "false");
    if (IsSet(SCF_DRM_ACTIVE) != drm) {
        if (drm) {
            SC_LOGD("Setting DRM flag");
            SetFlag(SCF_DRM_ACTIVE);
        } else {
            SC_LOGD("Clearing DRM flag");
            ClearFlag(SCF_DRM_ACTIVE);
        }

        SetFlag(SCF_DRM_FRESH); //new drm value should be sent to v4l2
    }
}
