...
{
    size_t size = 0, bpr, alignment = 0, ext_size=256;
    ...
    switch (format) {
        ...
        case HAL_PIXEL_FORMAT_RGBX_8888: //0x2
        case HAL_PIXEL_FORMAT_BGRA_8888: //0x5
                             ...
                             private_handle_t **hnd, int *stride)
{
    ...
    int err, planes, fd = -1, fd1 = -1, fd2 = -1, format2 = format;
    size_t luma_vstride;
    unsigned int heap_mask = _select_heap(usage);
        ...
        case HAL_PIXEL_FORMAT_EXYNOS_YV12_M: //0x11c
            {
                *stride = ALIGN(w, 32);
                luma_vstride = ALIGN(h, 16);
                luma_size = (luma_vstride * *stride) + ext_size;
                chroma_size = ( ALIGN( *stride / 2, 16) * (luma_vstride / 2) ) + ext_size;
                planes = 3;
                break;
            }
        case HAL_PIXEL_FORMAT_YV12: //0x32315659
