...
{
    LOG_DUMP("\t\tformat = 0x%x(%d), width = %u, height = %u, stride = %u, vstride = %u",
            h->format, h->format, h->width, h->height, h->stride, h->vstride);
}

char *dumpHandleStr(private_handle_t *h)
{
    char *dump = (char *) calloc(1024, sizeof(char));

    sprintf(dump, "0x%8x(%6d) | (%4u, %4u) | (%4u, %4u)",
            h->format, h->format, h->width, h->height, h->stride, h->vstride);
    return dump;
}
...
void dumpLayer(hwc_layer_1_t const *l)
{
    ...
    if(l->handle && !(l->flags & HWC_SKIP_LAYER)) {
        dump_hdl = dumpHandleStr(private_handle_t::dynamicCast(l->handle));
    }

    LOG_DUMP("%4d | %08x | %p | %02x | %05x | "
            "{%7.1f,%7.1f,%7.1f,%7.1f} | {%4d,%4d,%4d,%4d} | %s",
            ...
            l->displayFrame.bottom, dump_hdl);
}

void dumpConfig(decon_win_config &c)
{
    LOG_WCFG("\tstate = %u", c.state);
    if (c.state == c.DECON_WIN_STATE_BUFFER) {
        LOG_WCFG("\t\tfd_idma[0] = %d, fd_idma[1] = %d, fd_idma[2] = %d, plane_alpha(%d)"
...
enum decon_pixel_format halFormatToS3CFormat(int format)
{
    ...
    case HAL_PIXEL_FORMAT_EXYNOS_UNK:
