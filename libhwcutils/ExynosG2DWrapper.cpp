        ...
        unsigned long dstAddress, int is_lcd)
{
    ...
    if (!force_clear && src_handle->fd >= 0) {
        ...
        if (srcAddress) {
            ...
            srcYAddress = (unsigned long)ion_map(src_handle->fd, srcImageSize*srcG2d_bpp, 0);
            src_ion_mapped = true;
        }
...
#if 0 // J.Ferrer - 5.5.2016 - I don't want now to build libsecurepath
...
bool ExynosG2DWrapper::InitSecureG2D()
{
    ...
    if (mVirtualDisplay->mPhysicallyLinearBuffer == NULL) {
        ALOGV("init secure G2D");

#if 0 // J.Ferrer - 5.5.2016 - I don't want now to build libsecurepath
        ret = G2DDRM_Initialize();

        if (ret != 0) {
            ALOGE("%s: G2DDRM_Initialize failed", __func__);
            return false;
        }
#endif
    ...
    for (int i = 0; i < mDisplay->mAllocatedLayers; i++) {
        for (int j = 0; j < (int)NUM_GSC_DST_BUFS; j++) {
            if (mDisplay->mWinBufVirtualAddress[i][j]) {
                ion_unmap((void *)mDisplay->mWinBufVirtualAddress[i][j], mDisplay->mWinBufMapSize[i]);
...
int ExynosG2DWrapper::exynos5_g2d_buf_alloc(hwc_display_contents_1_t* contents)
{
    ...
    for (int i = 0; i < mDisplay->mG2dLayers; i++) {
        for (int j = 0; j < (int)NUM_GSC_DST_BUFS; j++) {
            ...
            vir_addr = (uint32_t) ion_map(buf_handle->fd, mDisplay->mWinBufMapSize[i], 0);
            if (vir_addr != (unsigned int)MAP_FAILED) {
...
int ExynosG2DWrapper::exynos5_config_g2d(hwc_layer_1_t &layer, private_handle_t *dst_handle, decon_win_config &cfg, int win_idx_2d, int win_idx)
{
#ifdef G2D_COMPOSITION
    int ret = 0;
