...
// vvvvv copied from kernel drivers/video/exynos/decon/vpp/vpp_drv.c vvvvv
static void vpp_get_align_variant(hwc_layer_1_t &layer, int format,
	uint32_t *offs, uint32_t *src_f, uint32_t *src_cr, uint32_t *dst_cr)
{
	if (isRotated(layer)) {
		if (is_yuv(format)) {
			*offs = *src_f = 4;
...
static void vpp_get_min_max_variant(hwc_layer_1_t &layer, int format,
		uint32_t *max_src, uint32_t *min_src_w, uint32_t *min_src_h,
		uint32_t *max_dst, uint32_t *min_dst_w, uint32_t *min_dst_h)
{
	if (isRotated(layer)) {
		if (is_yuv(format)) {
			*max_src = 2560;
...
// ^^^^^ copied from kernel drivers/video/exynos/decon/vpp/vpp_drv.c ^^^^^
        ...
        bool local_path, int loc_out_downscale)
{
    ...
    uint32_t offs, src_f, src_cr, dst_cr;
    uint32_t max_src, min_src_w, min_src_h;
    uint32_t max_dst, min_dst_w, min_dst_h;
    uint32_t f_w = 0, f_h = 0;

    if (handle->format == 0x121) { //TODO: find constant and add UNLIKELY
        f_w = handle->stride * 2;
        f_h = handle->vstride / 2;
    } else {
        f_w = handle->stride;
        f_h = handle->vstride;
    }

	vpp_get_align_variant(layer, format, &offs, &src_f, &src_cr, &dst_cr);
	vpp_get_min_max_variant(layer, format, &max_src, &min_src_w, &min_src_h, &max_dst, &min_dst_w, &min_dst_h);

    if ((!check_align(layer.sourceCrop.left, layer.sourceCrop.top, offs)) ||
        (!check_align(f_w, f_h, src_f)) ||
        (!check_align(WIDTH(layer.sourceCrop), HEIGHT(layer.sourceCrop), src_cr)) ||
        (!check_align(WIDTH(layer.displayFrame), HEIGHT(layer.displayFrame), dst_cr))) {
        ALOGE("%s: Alignment error", __FUNCTION__);
        return false;
    }

	if (WIDTH(layer.sourceCrop) > max_src || WIDTH(layer.sourceCrop) < min_src_w ||
		HEIGHT(layer.sourceCrop) > max_src || HEIGHT(layer.sourceCrop) < min_src_h) {
		ALOGE("%s: Unsupported source size", __FUNCTION__);
		return false;
	}

	if (WIDTH(layer.displayFrame) > max_dst || WIDTH(layer.displayFrame) < min_dst_w ||
		HEIGHT(layer.displayFrame) > max_dst || HEIGHT(layer.displayFrame) < min_dst_h) {
		ALOGE("%s: Unsupported dest size", __FUNCTION__);
		return false;
	}
...
{
    //ALOGD("%s: formatRequiresGsc(0x%x)=%d isScaled(%d) isTransformed(%d) !isXAligned(%d)", __FUNCTION__,
    //        format, formatRequiresGsc(format), isScaled(layer), isTransformed(layer), !isXAligned(layer, format));

    // atm only some transformed layers do need MPP processing. some yuv formats do need int mpp
    return isTransformed(layer) || !isFormatRgb(format);

    /*return false; //provisory
...
#endif
{
    ...
    if (!reconfigure && (mLastGSCLayerHandle == (uint32_t) (intptr_t)layer.handle)) {
        ...
        mLastGSCLayerHandle = (uint32_t) (intptr_t)layer.handle;
    }
#ifdef USES_VIRTUAL_DISPLAY
    }
#endif
